...
const char * const BITCOIN_CONF_FILENAME = "cmk.conf";
const char * const BITCOIN_PID_FILENAME = "cmkd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "cmk" is a composite category enabling all cmk-related debug output
            if(ptrCategory->count(string("cmk"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\cmkCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\cmkCore
    // Mac: ~/Library/Application Support/cmkCore
    // Unix: ~/.cmkCore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "cmkCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/cmkCore";
#else
    // Unix
    return pathRet / ".cmkCore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty cmk.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override cmk.conf
