...
cmk Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 1779.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 1777 for connections and services them.
...
However, in open source software such as cmk Core, where everyone uses
...
pay attention to for reviewers of cmk Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: cmk Core uses tinyformat, which is type safe. Leave them out to avoid confusion
