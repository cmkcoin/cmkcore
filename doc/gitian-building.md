...
*Setup instructions for a Gitian build of cmk Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the cmk
...
- [Building cmk Core](#building-cmk-core)
...
Clone the git repositories for cmk Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cmk-net/cmk-Core
```
...
in the cmk Core repository under 'Fetch and build inputs' to install sources which require
...
To build cmk Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the cmk Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cmk/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/cmk-net/cmk-Core
...
For example:
```bash
URL=https://github.com/crowning-/cmk.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cmk=${COMMIT} --url cmk=${URL} ../cmk/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cmk=${COMMIT} --url cmk=${URL} ../cmk/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cmk=${COMMIT} --url cmk=${URL} ../cmk/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the cmk git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../cmk/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clonehttps://github.com/cmk-net/cmk-Core.git

BTCPATH=/some/root/path/cmk.git
SIGPATH=/some/root/path/cmk-detached-sigs.git

./bin/gbuild --url cmk=${BTCPATH},signature=${SIGPATH} ../cmk/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cmk-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cmk-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/cmk-osx-build.assert
```
...
[cmk/gitian.sigs](https://github.com/cmk-net/cmk-Core.sigs/) repository, or if that's not possible to create a pull
request.
